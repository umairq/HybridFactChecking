...
class BaselineEmdeddingsOnlyModel(torch.nn.Module):
    def __init__(self, embedding_dim, num_entities, num_relations,dataset):
        ...
        self.shallom_width = int(1 * self.embedding_dim)
...
for cls in datasets_class:
    ...
    X_dataloader = DataLoader(torch.Tensor(dataset.idx_train_data).long(), batch_size=bat_size, num_workers=4, shuffle=True)
